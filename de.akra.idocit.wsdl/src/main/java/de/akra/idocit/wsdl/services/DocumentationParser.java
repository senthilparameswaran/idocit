...
public class DocumentationParser
{
	...
	/**
	 * Searches for the first documentation element in <code>docElems</code> with thematic
	 * elements as children.
	 * 
	 * @param docElems
	 *            List of documentation {@link Element}s.
	 * @return The first documentation {@link Element} with thematicgrid elements as
	...
	private static Documentation parseDocPartElement(Node docpartElem)
	{
		...
		{
			logger.log(Level.FINE, DocumentationParser.ERROR_DOCUMENTATION_ATTRIBUTE_NAME
					+ "=\"" + attr.getNodeValue() + "\"");
	...
	private static Documentation parseAddressees(Node docpartElem)
	{
		...
		for (int i = 0; i < childList.getLength(); i++)
		{
			...
			if (item.getNodeName().equals(ADDRESSEE_ELEMENT_NAME))
			{
				...
				Addressee addressee = DescribedItemUtils.findAddressee(attribute
						.getNodeValue());
				logger.log(Level.FINE,
	...
	private static String readTextFromAddresseElement(Node addresseeElement)
	{
		...
		for (int i = 0; i < nodes.getLength(); ++i)
		{
			...
			{
			case Node.TEXT_NODE:
				String unescapedText = StringEscapeUtils.unescapeHtml4(node
						.getNodeValue());
