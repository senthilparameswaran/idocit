...
/**
 * Generator that updates the {@link Definition} in the {@link WSDLInterfaceArtifact}
 * <code>wsdlIStructure</code> with the new {@link Documentation}s out of the
...
public class WSDLInterfaceGenerator
{
	 ...
	 * Logger.
	 */
	// private static Logger logger = Logger.getLogger(WSDLInterfaceGenerator.class
	// .getName());
	 ...
	 * <code>wsdlDefinition</code>. Old documentations are removed and all documentations
	 * in the {@link WSDLInterfaceArtifact} structure are added to the corresponding
	 * {@link WSDLElement} in the <code>wsdlDefinition</code>.
	 * 
	...
	public Definition updateDocumentationInDefinition() throws WSDLException
	{
		...
		this.documentationTagName = DOMUtils.getQualifiedValue(Constants.NS_URI_WSDL,
				Constants.ELEM_DOCUMENTATION, wsdlIStructure.getWsdlDefinition());

		// clean up WSDL definition
		removeAllDocumentationElementsWithDocparts(wsdlIStructure.getWsdlDefinition());

		// set for all Interfaces (PortTypes) the documentation into the wsdlDefinition
		...
		while (it.hasNext())
		{
			...
			Element newDocElem = generateDocumentationElement(documentationTagName,
					wsdlInterface.getDocumentations());
	...
	/**
	 * Update the documentations in the {@link javax.wsdl.Operation}s hold in the
	 * {@link WSDLOperation}s <code>operations</code> with the current
	 * {@link Documentation}s.
	 * 
	 * @param wsdlOperations
	 *            The {@link List} of {@link WSDLOperation}s with the new documentations
	 *            and containing reference to the corresponding
	 *            {@link javax.wsdl.Operation}.
	 */
	@SuppressWarnings("unchecked")
	private void updateOperationsDocumentations(List<WSDLOperation> wsdlOperations)
	{
		...
		for (WSDLOperation wsdlOperation : wsdlOperations)
		{
			...
			Element newDocElem = generateDocumentationElement(documentationTagName,
					wsdlOperation.getDocumentations());
			operation.addDocumentationElement(newDocElem);

			// updateMessagesDocumentations(wsdlOperation);

			// Update input message
			updateMessageDocumentation((WSDLMessage) wsdlOperation.getInputParameters());

			// Update output message
			updateMessageDocumentation((WSDLMessage) wsdlOperation.getOutputParameters());

			// Update fault messages
			for (WSDLMessage faultMsg : (List<WSDLMessage>) wsdlOperation.getExceptions())
			{
	...
	/**
	 * Update the documentations in the {@link Input}, {@link Output} or {@link Fault}
	 * depending on the held reference in the {@link WSDLMessage} <code>wsdlMessage</code>
	 * with the current {@link Documentation}s.
	 * 
	 * @param wsdlMessage
	 *            The {@link WSDLMessage} with the new documentations and containing
	 *            reference to the corresponding {@link Input}, {@link Output} or
	 *            {@link Fault}.
	 */
	private void updateMessageDocumentation(WSDLMessage wsdlMessage)
	{
		...
		Element newDocElem = generateDocumentationElement(documentationTagName, docparts);
		element.addDocumentationElement(newDocElem);
	}

	/**
	 * Adds all {@link Documentation}s of the {@link Parameters} structure into the
	 * {@link List} <code>docparts</code>.
	 * 
	 * @param docparts
	 *            The {@link List} into which all {@link Documentation}s are added.
	 * @param parameters
	 *            The {@link Parameters} whose and whose children's {@link Documentation}s
	 *            are added to <code>docparts</code>.
	 */
	...
	/**
	 * Adds all {@link Documentation}s of the {@link Parameter} structure into the
	 * {@link List} <code>docparts</code>.
	 * 
	 * @param docparts
	 *            The {@link List} into which all {@link Documentation}s are added.
	 * @param parameter
	 *            The {@link Parameter} whose and whose children's {@link Documentation}s
	 *            are added to <code>docparts</code>.
	 */
	...
	/**
	 * Removes all documentation {@link Element}s with docparts from the <b>whole</b> WSDL
	 * {@link Definition} <code>wsdlDefinition</code>.<br>
	 ...
	 */
	private void removeAllDocumentationElementsWithDocparts(Definition definition)
	{
		@SuppressWarnings("unchecked")
	...
	/**
	 * Removes all documentation {@link Element}s with docparts from the {@link PortType}
	 * <code>portType</code> and its subelements.
	 * 
	 * @param portType
	 ...
	 */
	private void removeAllDocumentationsWithDocpartsFromElement(WSDLElement wsdlElement)
	{
		Element docElem = null;

		// find all documentation elements with docparts and remove them
		while ((docElem = DocumentationParser.findDocElemWithDocpart(wsdlElement
				.getDocumentationElements())) != null)
		{
